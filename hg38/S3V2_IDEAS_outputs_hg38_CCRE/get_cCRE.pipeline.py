...
def get_cCRE_pipeline(script_dir, id_name, working_dir, bin_size):
	...
	print('get bed file done')

	### remove problematic NEU chr8 peaks
	b1 = call('for file in $(cat neu_files.txt.tmp); do cat $file | awk -F \'\t\' -v OFS=\'\t\' \'{if ($1!="chr8") print $0}\' > $file.tmp && mv $file.tmp $file; done', shell=True)	
	#b1 = call('cat S3V2_IDEAS_hg38_ccre2.ct.list.txt | awk \'{if ($1!="NEU_C0011IH2" && $1!="NEU_C001UYH1") print $0}\' > tmp1111 && mv tmp1111 S3V2_IDEAS_hg38_ccre2.ct.list.txt', shell=True)
	...
	### merge 3 and 32 because only using 3 may miss some important example cCREs
	for ct in ct_list:
		e = call('cp '+id_name+'.'+ ct +'.32.bed ' +id_name+'.'+ ct +'.3.bed', shell=True)
	...
	print(' remove unreproducible ones')
	for st in HS:
