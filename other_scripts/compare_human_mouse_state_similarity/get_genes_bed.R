...
mm10_gene_sample = mm10_gene[is.element(mm10_gene[,5], c('Slc4a1', 'Rps19')),]
...
for (i in 1:nrow(mm10_gene_sample_tss)) {
  write.table(mm10_gene_sample_tss[i,], paste0('mm10.gene.', mm10_gene_sample_tss[i,5], '.bed'), col.names = F, row.names = F, quote = F, sep = '\t')
}
...
echo $i
# intersect state bed
bedtools intersect -a $mm10_state_set -b 'mm10.gene.'$i'.bed' -wa -u > 'mm10.gene.'$i'.matched_ct.state.bed'
# get state similarity matrix
cat 'hg38.gene.'$hg38_gene'.bed' 'mm10.gene.'$i'.bed'
Rscript $script_dir/get_hg38_mm10.cor.heatmap.simple.R 'hg38.gene.'$hg38_gene'.matched_ct.state.bed' 'mm10.gene.'$i'.matched_ct.state.bed' $hg38_gene'.'$i'.cor.heatmap.png' $para_file $feature_num 
# rm matched_ct.state.bed
rm 'mm10.gene.'$i'.matched_ct.state.bed'
