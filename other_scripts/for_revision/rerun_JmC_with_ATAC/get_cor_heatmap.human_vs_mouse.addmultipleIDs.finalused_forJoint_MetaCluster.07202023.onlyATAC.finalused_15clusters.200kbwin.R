...
dh0 = read.table('S3V2_IDEAS_hg38_ccre2.cCRE.M.notall0.withid.ATACsig.withid.txt', header=T, sep='\t')
...
png('ATAC_only/PCA.plot.raw.png')
...
png(paste0('ATAC_only/PCA.plot.raw.1.',ii,'.png'))
pc_i = 1
pc_j = ii
...
pdf('ATAC_only/KMPCA.Joint.1.cluster.pdf', height=30)
...
pdf('ATAC_only/cCRE_esRP_KM.determineK.pdf')
...
pdf('ATAC_only/KM.reproducible.p.hist.pdf')
...
pdf('ATAC_only/abs_HM_count_mean_log2FC.hist.pdf')
...
png('ATAC_only/abs_HM_count_mean_log2FC.reproducible.png')
...
pdf('ATAC_only/KMPCA.Joint.reproducible.cross_spec.cluster.pdf', height=10, width=6)
...
pdf('ATAC_only/KMPCA.Joint.reproducible.cluster.pdf', height=12, width=6)
plot_color_lim = 15
...
pdf('ATAC_only/KMnotPCA.Joint.reproducible.cluster.pdf', height=12, width=6)
plot_color_lim = 15
breaksList = seq(-plot_color_lim, plot_color_lim, by = 0.001)
my_colorbar=colorRampPalette(c('blue', 'white', 'red'))(n = length(breaksList))
...
set.seed(2019)
pdf('ATAC_only/dhs_dms_ctmerge_shared_reorder_meansig.hist.pdf')
...
pdf('ATAC_only/KM_by_KM_SameClu_count.reproducible.pdf', height=10, width=10)
...
pdf('ATAC_only/KMPCA.Joint.cluster.reproducible.pdf', height=12)
...
pdf('ATAC_only/KMPCA.Joint.cluster.reproducible_H.pdf', height=10)
...
pdf('ATAC_only/KMPCA.Joint.cluster.reproducible_M.pdf', height=10)
...
png('ATAC_only/cCRE_id_HM_count.png')
plot_lim = c(5, 50000)
...
pdf('ATAC_only/KMPCA.Joint.cluster.reproducible.Jmet.pdf', height=6)
...
pdf('ATAC_only/KMPCA.Joint.cluster.reproducible.Jmet.OD.pdf', height=6)
dhs_dms_ctmerge_shared_reorder_meansig_Jmet_meansig_plot = dhs_dms_ctmerge_shared_reorder_meansig_Jmet_meansig
dhs_dms_ctmerge_shared_reorder_meansig_Jmet_meansig_plot = dhs_dms_ctmerge_shared_reorder_meansig_Jmet_meansig_plot[,c('MON', 'GMP', 'HSC','CMP','ERY','MEP','MK','NK','TCD4','TCD8','CLP','AVE','B')]
plot_color_lim = max(dhs_dms_ctmerge_shared_reorder_meansig_Jmet_meansig_plot)
breaksList = seq(min(dhs_dms_ctmerge_shared_reorder_meansig_Jmet_meansig_plot), plot_color_lim, by = 0.001)
...
write.table(dh_with_JointClusterID_mat[,c(1:3,4,6,7)], 'ATAC_only/S3V2_IDEAS_hg38_ccre2.cCRE.M.notall0.rmallNEU.withid.cCRE_ID.SRC_ID.JmC_ID.bed', quote=F, sep='\t', col.names=T, row.names=F)
write.table(dm_with_JointClusterID_mat[,c(1:3,4,6,7)], 'ATAC_only/S3V2_IDEAS_mm10_ccre2.cCRE.M.notall0.withid.cCRE_ID.SRC_ID.JmC_ID.bed', quote=F, sep='\t', col.names=T, row.names=F)
...
write.table(dh_with_JointClusterID_mat, 'ATAC_only/S3V2_IDEAS_hg38_ccre2.cCRE.M.notall0.rmallNEU.withid.coe_mat.PDmerged.clusterID.JclusterID.txt', quote=F, sep='\t', col.names=T, row.names=F)
write.table(dm_with_JointClusterID_mat, 'ATAC_only/S3V2_IDEAS_mm10_ccre2.cCRE.M.notall0.withid.coe_mat.PDmerged.clusterID.JclusterID.txt', quote=F, sep='\t', col.names=T, row.names=F)

write.table(dh_with_JointClusterID_mat[,c(1:3,7)], 'ATAC_only/S3V2_IDEAS_hg38_ccre2.cCRE.M.notall0.rmallNEU.withid.clusterID.JclusterID.bed', quote=F, sep='\t', col.names=T, row.names=F)
write.table(dm_with_JointClusterID_mat[,c(1:3,7)], 'ATAC_only/S3V2_IDEAS_mm10_ccre2.cCRE.M.notall0.withid.clusterID.JclusterID.bed', quote=F, sep='\t', col.names=T, row.names=F)
...
pdf('ATAC_only/KMPCA.Joint.cluster.reproducible.Jmet.cCRE_count.pdf', height=5,width=3)
...
colnames(meta_cluster_mat) = rownames(JointCluster_count_HM)
rownames(meta_cluster_mat) = shared_genes
write.table(cbind(shared_genes, round(meta_cluster_mat,5)), 'ATAC_only/Human_Mouse_shared_genes.Jmeta.enrich.100KB.txt', quote=F, sep='\t', col.names=T, row.names=F)
write.table(cCRE_H_Jmeta, 'ATAC_only/cCRE.Gene100KB.hg38.bed', quote=F, sep='\t', col.names=T, row.names=F)
write.table(cCRE_M_Jmeta, 'ATAC_only/cCRE.Gene100KB.mm10.bed', quote=F, sep='\t', col.names=T, row.names=F)
#################################################
...
pdf('ATAC_only/KM_gene_Jmet_enrich.determineK.pdf')
...
pdf('ATAC_only/meta_cluster_mat_log2_df.hist.1.pdf')
...
2.270142
write.table(meta_cluster_mat_log2_df, 'ATAC_only/Human_Mouse_shared_genes.Jmeta.enrich.100KB.txt', quote=F, sep='\t', col.names=T, row.names=F)
#################################################


#################################################
### plot heatmap
png('ATAC_only/GeneGroup_by_JMeta.Joint.cluster.png', width = 1000, height = 600)
...
pdf('ATAC_only/GeneGroup_by_JMeta.Joint.cluster.pdf', height=9 , width = 12)
meta_cluster_mat_GeneGroup_Jmet = c()
used_order = Jmet_order
for (coli in used_order){
  meta_cluster_mat_GeneGroup_Jmet = cbind(meta_cluster_mat_GeneGroup_Jmet, meta_cluster_mat_log2_km_mean[,colnames(meta_cluster_mat)==coli])
...
write.table(meta_cluster_mat_GeneGroup_Jmet_ID_pairs, 'ATAC_only/enriched_JmC_ID_KM_ID.100KB.txt', sep='\t', quote=F, col.names=F, row.names=F)
...
pdf('ATAC_only/Gene_by_JMeta.Joint.cluster.pdf', height=5.8, width=9)
...
dev.off()
pdf('ATAC_only/Gene_by_JMeta.Joint.cluster_H.pdf', height=5, width=9)
...
dev.off()
pdf('ATAC_only/Gene_by_JMeta.Joint.cluster_M.pdf', height=5, width=9)
...
png(paste0('ATAC_only/Gene_enrich.heatscatterplot.',colnames(meta_cluster_mat_plot_H)[i],'.100KB.png'))
...
#A_gene_enrich = log2(meta_cluster_mat_plot_H_vec*meta_cluster_mat_plot_M_vec_adj)
used_id = sample(length(meta_cluster_mat_plot_H_vec), 10000)
png(paste0('ATAC_only/Gene_enrich.heatscatterplot.all.png'))
#heatscatter(A_gene_enrich[used_id], M_gene_enrich[used_id], ylim = c(-max(abs(M_gene_enrich)), max(abs(M_gene_enrich))))
#abline(h=0)
plot_lim_i = c(min(c(meta_cluster_mat_plot_H_vec, meta_cluster_mat_plot_M_vec)), max(c(meta_cluster_mat_plot_H_vec, meta_cluster_mat_plot_M_vec)))+1
print(cor(meta_cluster_mat_plot_H_vec, meta_cluster_mat_plot_M_vec))
...
png(paste0('ATAC_only/Gene_enrich.blackscatterplot.all.png'))
#heatscatter(A_gene_enrich[used_id], M_gene_enrich[used_id], ylim = c(-max(abs(M_gene_enrich)), max(abs(M_gene_enrich))))
#abline(h=0)
plot_lim_i = c(min(c(meta_cluster_mat_plot_H_vec, meta_cluster_mat_plot_M_vec)), max(c(meta_cluster_mat_plot_H_vec, meta_cluster_mat_plot_M_vec)))+1
print(cor(meta_cluster_mat_plot_H_vec, meta_cluster_mat_plot_M_vec))
...
dev.off()
###
png('ATAC_only/Gene_enrich.MAplot.png')
meta_cluster_mat_plot_H_vec = as.numeric(c(meta_cluster_mat_plot_H[,used_col_for_plotting_MAplot]))
meta_cluster_mat_plot_M_vec = as.numeric(c(meta_cluster_mat_plot_M[,used_col_for_plotting_MAplot]))
enrich_lm_adj = lm(meta_cluster_mat_plot_H_vec~meta_cluster_mat_plot_M_vec)
#meta_cluster_mat_plot_M_vec_adj = meta_cluster_mat_plot_M_vec * enrich_lm_adj$coefficients[2] + enrich_lm_adj$coefficients[1]
...
png('ATAC_only/Gene_enrich.MAplot.black.png')
meta_cluster_mat_plot_H_vec = as.numeric(c(meta_cluster_mat_plot_H[,used_col_for_plotting_MAplot]))
meta_cluster_mat_plot_M_vec = as.numeric(c(meta_cluster_mat_plot_M[,used_col_for_plotting_MAplot]))
enrich_lm_adj = lm(meta_cluster_mat_plot_H_vec~meta_cluster_mat_plot_M_vec)
#meta_cluster_mat_plot_M_vec_adj = meta_cluster_mat_plot_M_vec * enrich_lm_adj$coefficients[2] + enrich_lm_adj$coefficients[1]
