...
do
    ...
    cut -f 1-3 ${bed_file_ref_all} | bedtools shuffle -g ${GENOME_SIZE} -noOverlapping -i - \
...
line_num=$(cat $bed_file | wc -l)
echo $line_num

rm ${bed_file}_matched.bed
...
do
    ...
    awk -v FS="\t" -v OFS="\t" -v line=${i} 'FNR==line {print FILENAME, $0}' ${bed_file}*fabed | sort -k5nr > ${Proc_file}
    ...
    #just 1 line after will be selected with similar GC content. So, larger pool get more similar GC sequence, but slower :)
    if [ ${ref_line} -eq 1 ]
    then
