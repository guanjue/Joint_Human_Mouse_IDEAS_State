...
#dss_mean_vec_cor_dist = dist(allct_ct_5group_mat)
...
new_id_meta_1[is.element(new_id, c(11,12))]  = max(new_id_meta_1)+1
new_id_meta_1[is.element(new_id, c(37,38))]  = max(new_id_meta_1)+1
table(new_id[new_id_meta==14])
table(new_id[new_id_meta==20])
new_id_meta_1[is.element(new_id, c(89,90))]  = max(new_id_meta_1)+1
table(new_id[new_id_meta==16])
table(new_id[new_id_meta==17])
new_id_meta_1[is.element(new_id, c(24,32,33,34,15,16,20))]  = max(new_id_meta_1)+1
table(new_id[new_id_meta==20])
new_id_meta_1[is.element(new_id, c(48,49,91,92))]  = max(new_id_meta_1)+1
table(new_id[new_id_meta==20])
table(new_id[new_id_meta==21])
new_id_meta_1[is.element(new_id, c(50,85,93,77,78))]  = max(new_id_meta_1)+1
...
### plot mean matrix
library(pheatmap)
coe = read.table('S3V2_IDEAS_hg38_ccre2.cCRE.M.notall0.rmallNEU.withid.coe_mat.PDmerged.clusterID.txt', header=T, sep='\t')
