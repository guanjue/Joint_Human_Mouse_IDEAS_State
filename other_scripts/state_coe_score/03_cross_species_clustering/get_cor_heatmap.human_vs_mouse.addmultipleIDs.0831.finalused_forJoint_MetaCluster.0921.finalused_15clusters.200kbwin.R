...
colnames(meta_cluster_mat) = rownames(JointCluster_count_HM)
rownames(meta_cluster_mat) = shared_genes
write.table(cbind(shared_genes, round(meta_cluster_mat,5)), 'Human_Mouse_shared_genes.Jmeta.enrich.100KB.txt', quote=F, sep='\t', col.names=T, row.names=F)
write.table(cCRE_H_Jmeta, 'cCRE.Gene100KB.hg38.bed', quote=F, sep='\t', col.names=T, row.names=F)
write.table(cCRE_M_Jmeta, 'cCRE.Gene100KB.mm10.bed', quote=F, sep='\t', col.names=T, row.names=F)
#################################################
...
box()
dev.off()
write.table(meta_cluster_mat_log2_df, 'Human_Mouse_shared_genes.Jmeta.enrich.100KB.txt', quote=F, sep='\t', col.names=T, row.names=F)
#################################################
...
write.table(meta_cluster_mat_GeneGroup_Jmet_ID_pairs, 'enriched_JmC_ID_KM_ID.100KB.txt', sep='\t', quote=F, col.names=F, row.names=F)
...
png(paste0('Gene_enrich.heatscatterplot.',colnames(meta_cluster_mat_plot_H)[i],'.100KB.png'))
