...
setwd('/gpfs/group/yzz2/default/legacy/group/projects/vision_joint_human_mouse/coefficients_human_23ES_with_cCRE_with_corfilter/')
...
ct_reorder = c(3,1,2,4,23,8,5,9,19,6,14,12,22,7,16,10,11,17,21,15,20,18,13)
...
### get state coverage
sp_all = c()
for (i in c(0:(nstate_0-1))){
ss = read.table(paste('../coefficients_human_23ES_with_cCRE_with_corfilter/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.Nkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i = c()
for (ct in common_ct){
...
sp_all_dist = c()
for (i in c(0:(nstate_0-1))){
ss = read.table(paste('../coefficients_human_23ES_with_cCRE_with_corfilter/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.NHkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i = c()
for (ct in common_ct){
ct_pos_sp = which(sp_list==ct)
...
### correlation heatmap
sp_mat_for_corheatmap = sp_all_PD_log[,-c(1,(nstate_0+1))]
...
lm_raw_all = lm(dmslog_noMean[used_idtry]~sp_all_PD_log[,-c(1,(nstate_0+1))])
...
rownames(eRP_mat_human_raw) = 0:(nstate_0-1)
...
pca_all <- prcomp(sp_all_PD_log[,-c(1,(nstate_0+1))], center = F,scale. = F)
...
rownames(eRP_mat_human) = 0:(nstate_0-1)
...
for (i in ct_pos_sp){
sp_mat_tmp = sp_mat_tmp+sp_corfiltered[,seq(1,39*(nstate_0), by=39)+i-1]
}
...
colnames(sp_corfiltered_all) = c(0:(nstate_0-1))
...
### PCA lm
used_idtry = dmslog_noMean>(-1000000)
sp_all_PD_corfilter_log_new = sp_all_PD_corfilter_log[,-c(1,(nstate_0+1))] %*% pca0_rotation
