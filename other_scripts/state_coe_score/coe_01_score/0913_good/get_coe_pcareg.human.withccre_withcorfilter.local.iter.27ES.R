...
setwd('/gpfs/group/yzz2/default/legacy/group/projects/vision_joint_human_mouse/coefficients_human_27ES_with_cCRE_with_corfilter/')
...
### get RNA
d = read.table('/gpfs/group/yzz2/default/legacy/group/projects/vision_joint_human_mouse/coefficients_human_27ES_with_cCRE_with_corfilter/HumanVISION_RNAseq_hg38_genes_tpm.idsort.protein_coding.txt', header=F)
ds = d[,-c(1:6)]
...
sp_all = c()
for (i in c(0:26)){
ss = read.table(paste('../coefficients_human_27ES_with_cCRE_with_corfilter/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.Nkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i = c()
for (ct in common_ct){
...
sp_all_dist = c()
for (i in c(0:26)){
ss = read.table(paste('../coefficients_human_27ES_with_cCRE_with_corfilter/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.NHkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i = c()
for (ct in common_ct){
ct_pos_sp = which(sp_list==ct)
...
### correlation heatmap
sp_mat_for_corheatmap = sp_all_PD_log[,-c(1,28)]
colnames(sp_mat_for_corheatmap) = c(paste('P:', 1:26), paste('D:', 1:26))
...
lm_raw_all = lm(dmslog_noMean[used_idtry]~sp_all_PD_log[,-c(1,28)])
Bpca_raw_all = lm_raw_all$coefficients[-1]
eRP_mat_human_raw = cbind(Bpca_raw_all[1:26],Bpca_raw_all[27:52])
eRP_mat_human_raw = rbind(c(0,0), eRP_mat_human_raw)
colnames(eRP_mat_human_raw) = c('P','D')
rownames(eRP_mat_human_raw) = 0:26
...
rank = c(26,13,22,13,8,27,24,20,25,17,12,15,4,10,16,3,1,6,2,5,21,7,23,11,19,9,18)
...
pca_all <- prcomp(sp_all_PD_log[,-c(1,28)], center = F,scale. = F)
...
eRP_mat_human = cbind(Bpca_all[1:26],Bpca_all[27:52])
eRP_mat_human = rbind(c(0,0), eRP_mat_human)
colnames(eRP_mat_human) = c('P','D')
rownames(eRP_mat_human) = 0:26
...
rank = c(26,14,22,13,8,27,24,20,25,17,12,15,4,10,16,3,1,6,2,5,21,7,23,11,19,9,18)
breaksList = seq(-plot_lim_P, plot_lim_P, by = 0.001)
my_colorbar=colorRampPalette(c('blue', 'white', 'red'))(n = length(breaksList))
col_breaks = c(seq(0, 2000,length=33))
...
sp_corfiltered_all = matrix(0, nrow=dim(sp_corfiltered)[1]*length(common_ct), ncol=27)
k=0
for (ct in common_ct){
print(ct)
ct_pos_sp = which(sp_list==ct)
sp_mat_tmp = matrix(0, nrow=dim(sp_corfiltered)[1], ncol=27 )
for (i in ct_pos_sp){
sp_mat_tmp = sp_mat_tmp+sp_corfiltered[,seq(1,39*27, by=39)+i-1]
}
...
colnames(sp_corfiltered_all) = c(0:26)
...
### PCA lm
used_idtry = dmslog_noMean>(-1000000)
sp_all_PD_corfilter_log_new = sp_all_PD_corfilter_log[,-c(1,28)] %*% pca0_rotation
