...
pca_all_P <- prcomp(sp_all_PD_log[,c(2,24)], center = F,scale. = F)
pca_all_P0_rotation = pca_all_P$rotation 
exp_var = summary(pca_all_P)$importance[2,]
exp_var_sum = 0
k = 0
...
lm_all_P = lm(dmslog_noMean[used_idtry]~pca_all_P$x[used_idtry,c(1:pcn)])
Bpca_all_P = pca_all_P$rotation[,c(1:pcn)] %*% lm_all$coefficients[-1]

alpha = lm_all_P$coefficients[1]

pred_P = lm_all_P$fitted.value #+ mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
#pca_all$x[,1:pcn] %*% lm_all$coefficients + mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
R2(dmslog_noMean[used_idtry], pred_P)


set.seed(2019)
used_id = sample(length(pred_P), 50000)
plot_lim = plot_lim_all#c(min(c(dmslog_noMean+mean(ds_qt_all_log), pred)), max(c(dmslog_noMean+mean(ds_qt_all_log), pred)))

png('pred_obs.tpm1.P.png')
...
heatscatter(as.vector(pred_P)[used_id], as.vector(dmslog_noMean)[used_id], xlim=plot_lim, ylim=plot_lim, main=R2(dmslog_noMean[used_idtry], pred_P))
