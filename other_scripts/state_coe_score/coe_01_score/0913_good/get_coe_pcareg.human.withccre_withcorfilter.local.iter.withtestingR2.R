...
set.seed(2019)
test_id = sample(dim(ds_qt)[1], round(dim(ds_qt)[1]/10))


for (ct in common_ct){
ct_pos_rna = which(rna_list==ct)
if (length(ct_pos_rna)>1){
rna_mat_tmp = rowMeans(ds_qt[-test_id,ct_pos_rna])
rna_mat_tmp_test = rowMeans(ds_qt[test_id,ct_pos_rna])
}else{
rna_mat_tmp = ds_qt[-test_id,ct_pos_rna]
rna_mat_tmp_test = ds_qt[test_id,ct_pos_rna]
}
ds_qt_all = c(ds_qt_all, rna_mat_tmp)
ds_qt_all_test = c(ds_qt_all_test, rna_mat_tmp_test)
}

ds_qt_all_log = log(ds_qt_all+smallnum)
ds_qt_all_log_test = log(ds_qt_all_test+smallnum)
...
sp_mat_tmp = rowMeans(sp[-test_id,ct_pos_sp])
sp_mat_tmp_test = rowMeans(sp[test_id,ct_pos_sp])
}else{
sp_mat_tmp = sp[-test_id,ct_pos_sp]
sp_mat_tmp_test = sp[test_id,ct_pos_sp]
}
sp_i = c(sp_i, sp_mat_tmp)
sp_i_test = c(sp_i_test, sp_mat_tmp_test)
}
print(i)
sp_all = cbind(sp_all, sp_i)
sp_all_test = cbind(sp_all_test, sp_i_test)
}
...
sp_all_dist_test = cbind(sp_all_dist_test, sp_i_test)
}
...
sp_all_dist_log_test = log(sp_all_dist_test+smallnum_sp)

#sp_all_dist_log = sp_all_dist

### prepare pca
sp_all_PD_log = cbind(sp_all_log, sp_all_dist_log)
sp_all_PD_log_test = cbind(sp_all_log_test, sp_all_dist_log_test)

### match high low gene means
dmslog_noMean = ds_qt_all_log# - mean(ds_qt_all_log)
dmslog_noMean_test = ds_qt_all_log_test# - mean(ds_qt_all_log)
...
pred_test = sp_all_PD_log_test[,-c(1,25)] %*% pca_all$rotation[,c(1:pcn)] %*% lm_all$coefficients[-1] + lm_all$coefficients[1]
#pca_all$x[,1:pcn] %*% lm_all$coefficients + mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
R2(dmslog_noMean_test, pred_test)
0.5795881
