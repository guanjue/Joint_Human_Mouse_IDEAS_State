...
### PCA lm
used_idtry = c(ds_qt_all_log_l, ds_qt_all_log_h)>1
...
R2(c(ds_qt_all_log_l,ds_qt_all_log_h)[used_idtry], pred)
0.4914278
0.4093814
0.4121036
...
plot_lim = c(-1,7)#c(min(c(dmslog_noMean+mean(ds_qt_all_log), pred)), max(c(dmslog_noMean+mean(ds_qt_all_log), pred)))
...
lm_raw = lm(c(dmslog_noMean_l,dmslog_noMean_h)~rbind(sp_all_PD_log_l,sp_all_PD_log_h)[,-c(1,25)]-1)
