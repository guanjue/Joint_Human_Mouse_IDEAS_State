...
setwd('/gpfs/group/yzz2/default/legacy/group/projects/vision_joint_human_mouse/coefficients_human_with_cCRE/')
...
sp_all_dist_bg = c()
for (i in c(0:23)){
ss = read.table(paste('../coefficients_human_with_cCRE/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.NHkbupdownexp_both.S',i,'.bed', sep=''), header=F)
ss = read.table(paste('../coefficients_human/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.NHkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
spm = rowMeans(sp)
sp_i = c()
for (ct in common_ct){
...
print(i)
sp_all_dist_bg = cbind(sp_all_dist_bg, sp_i)
}

sp_all_dist_bg = sp_all_dist_bg-sp_all_dist
sp_all_dist_bg[sp_all_dist_bg<0] = 0
sp_all_dist_bg_s = sp_all_dist_bg+smallnum_sp
...
#sp_all_dist_adj = t(t(sp_all_dist)/colMeans(sp_all_dist_bg_s))*mean(sp_all_dist_bg_s)
sp_all_dist_adj = (sp_all_dist)/sp_all_dist_bg_s*mean(sp_all_dist_bg_s)

sp_all_dist_log = log(sp_all_dist_adj+smallnum_sp)


print(mean(sp_all_dist_bg_s+smallnum_sp))
...
### correlation heatmap
sp_mat_for_corheatmap = sp_all_PD_log[,-c(1,25)]
colnames(sp_mat_for_corheatmap) = c(paste('P:', 1:23), paste('D:', 1:23))
...
lm_raw_all = lm(dmslog_noMean[used_idtry]~sp_all_PD_log[,-c(1,25)])
Bpca_raw_all = lm_raw_all$coefficients[-1]
eRP_mat_human_raw = cbind(Bpca_raw_all[1:23],Bpca_raw_all[24:46])
...
ss00 = read.table(paste('../coefficients_human_with_cCRE_with_corfilter/bp24_v2_cCRE_max1.cCRE.expand.M.withid.S0.mat.txt', sep=''), header=F)

### get bg
sp_sig_pred_ct_bg = matrix(0, nrow=dim(ss00)[1], ncol=dim(ss00)[2])
for (i in c(0:23)){
print(i)
ss = read.table(paste('../coefficients_human_with_cCRE_with_corfilter/bp24_v2_cCRE_max1.cCRE.expand.M.withid.S',i,'.mat.txt', sep=''), header=F)
sp = (ss[,-c(1:4)])
#sp_sig = sp*eRP_mat_human[i+1,2]
sp_sig = rowMeans(sp)
sp_sig_pred_ct_bg[,i+1] = sp_sig
}

sp_sig_pred_ct_pk = matrix(0, nrow=dim(ss00)[1], ncol=dim(ss00)[2])
for (i in c(0:23)){
print(i)
ss = read.table(paste('../coefficients_human_with_cCRE_with_corfilter/bp24_v2_cCRE_max1.cCRE.M.withid.S',i,'.mat.txt', sep=''), header=F)
sp = (ss[,-c(1:4)])
#sp_sig = sp*eRP_mat_human[i+1,2]
sp_sig = rowMeans(sp)
sp_sig_pred_ct_pk[,i+1] = sp_sig
}

sp_sig_pred_ct_bg = sp_sig_pred_ct_bg-sp_sig_pred_ct_pk
sp_sig_pred_ct_bg[sp_sig_pred_ct_bg<0] = 0
sp_sig_pred_ct_bg_s = sp_sig_pred_ct_bg+smallnum_sp
...
sp = (ss[,-c(1:4)])
#sp_sig = sp*eRP_mat_human[i+1,2]
sp_adj = sp / colMeans(sp_sig_pred_ct_bg_s)[i+1] * mean(sp_sig_pred_ct_bg_s)
sp_sig = log(sp_adj+smallnum_sp)*eRP_mat_human[i+1,2]
...
gene_withccre_id = read.table('../coefficients_human_with_cCRE_with_corfilter/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.NHkbupdownexp.withccreid.bed', header=F)[,5]
...
ss = read.table(paste('../coefficients_human_with_cCRE_with_corfilter/bp24_v2_cCRE_max1.cCRE.M.withid.S0.mat.txt', sep=''), header=F)
ss_all = (ss[,-c(1:4)])
for (i in c(1:23)){
print(i)
ss = read.table(paste('../coefficients_human_with_cCRE_with_corfilter/bp24_v2_cCRE_max1.cCRE.M.withid.S',i,'.mat.txt', sep=''), header=F)
...
### PCA lm
used_idtry = dmslog_noMean>(-1000000)
sp_all_PD_corfilter_log_new = sp_all_PD_corfilter_log[,-c(1,25)] %*% pca0_rotation
