cd /gpfs/group/yzz2/default/legacy/group/projects/vision_joint_human_mouse/coefficients_mouse_withccre
...
bedtools intersect -a VISION_mouse_ES_S3V2_7mk_forjoint_HP_MP.state.bed -b MouseVISION_RNAseq_mm10_gene.protein_coding.Nkbupdownexp.bed -wa -u \
> VISION_mouse_ES_S3V2_7mk_forjoint_HP_MP.state.gene.bed
cut -f4 VISION_mouse_ES_S3V2_7mk_forjoint_HP_MP.state.gene.bed > VISION_mouse_ES_S3V2_7mk_forjoint_HP_MP.state.ingene
...
do
echo $j
tail -n+2 VISION_mouse_ES_S3V2_7mk_forjoint_HP_MP.state.ingene \
| awk -F ' ' -v OFS='\t' -v c=$j -v s=$i '{if ($c==s) print $2,$3,$4}' > ct$j.s$i.bed
bedtools intersect -a MouseVISION_RNAseq_mm10_gene.protein_coding.Nkbupdownexp.bed -b ct$j.s$i.bed -c > ct$j.s$i.bed.count.bed
