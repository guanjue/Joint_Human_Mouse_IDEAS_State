...
### get RNA
d = read.table('coefficients_mouse/RNA_gene.mm10.sigmat.txt', header=F)
ds0 = d[,-c(1:2)]
dsmm10 = 2^(ds0)
...
pdf('coefficients_mouse/dms_log_rna.hist.mouse.pdf', width=5, height=5)
...
sp_all_h = c()
for (i in c(0:23)){
ss = read.table(paste('coefficients_mouse/MouseVISION_RNAseq_mm10_gene.protein_coding.Nkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i_l = c()
sp_i_h = c()
for (ct in common_ct){
...
sp_all_dist_h = c()
for (i in c(0:23)){
ss = read.table(paste('coefficients_mouse/MouseVISION_RNAseq_mm10_gene.protein_coding.NHkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i_l = c()
sp_i_h = c()
for (ct in common_ct){
...
### prepare pca
sp_all_PD_log_l_m = cbind(sp_all_log_l, sp_all_dist_log_l)
sp_all_PD_log_h_m = cbind(sp_all_log_h, sp_all_dist_log_h)

dmslog_noMean_l_m = ds_qt_all_log_l - mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
dmslog_noMean_h_m = ds_qt_all_log_h - mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
...
rna_l = mod$classification==1
rna_h = mod$classification==2
pdf('coefficients_human/dms_log_rna.hist.pdf', width=5, height=5)
plot(mod, what = "density", data = dms_log, breaks = 30)
abline(v=gmm_thresh, col='red', lwd=2, lty=2)
dev.off()
...
sp_all_h = c()
for (i in c(0:23)){
ss = read.table(paste('coefficients_human/HumanVISION_RNAseq_hg38_gene.idsort.protein_coding.Nkbupdownexp.S',i,'.bed', sep=''), header=F)
sp = (ss[,-c(1:3)])
sp_i_l = c()
sp_i_h = c()
for (ct in common_ct){
...
### prepare pca
sp_all_PD_log_l_h = cbind(sp_all_log_l, sp_all_dist_log_l)
sp_all_PD_log_h_h = cbind(sp_all_log_h, sp_all_dist_log_h)

### match high low gene means
dmslog_noMean_l_h = ds_qt_all_log_l - mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
dmslog_noMean_h_h = ds_qt_all_log_h - mean(c(ds_qt_all_log_l, ds_qt_all_log_h))



pca_all <- prcomp(rbind(sp_all_PD_log_l_m,sp_all_PD_log_h_m, sp_all_PD_log_l_h,sp_all_PD_log_h_h)[,-c(1,25)], center = F,scale. = F)
...
lm_all = lm(c(dmslog_noMean_l_m,dmslog_noMean_h_m, dmslog_noMean_l_h, dmslog_noMean_h_h)~pca_all$x[,1:pcn]-1)
...
pred = pca_all$x[,1:pcn] %*% lmvar_all$coefficients_mu + mean(c(ds_qt_all_log_l, ds_qt_all_log_h))
R2(c(ds_qt_all_log_l,ds_qt_all_log_h)+mean(c(ds_qt_all_log_l, ds_qt_all_log_h)), pred)
