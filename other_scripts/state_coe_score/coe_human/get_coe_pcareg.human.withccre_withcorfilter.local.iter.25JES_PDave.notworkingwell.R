...
setwd('/Users/guanjuexiang/Downloads/coe_analysis/')
...
sp_all_PD_log = cbind((sp_all_dist_log ))
...
colnames(sp_mat_for_corheatmap) = c(paste('D:', 1:(state_n-1)))
...
lm_raw_all = lm(dmslog_noMean[used_idtry]~sp_all_PD_log[,-c(1)])
Bpca_raw_all = lm_raw_all$coefficients[-1]
eRP_mat_human_raw = cbind(Bpca_raw_all[1:(state_n-1)])
eRP_mat_human_raw = rbind(c(0), eRP_mat_human_raw)
colnames(eRP_mat_human_raw) = c('D')
rownames(eRP_mat_human_raw) = 0:(state_n-1)

plot_lim_P = max(abs(eRP_mat_human_raw[,1]))
plot_lim_PD = max(abs(eRP_mat_human_raw))
pdf(paste(output_folder, '/statep_rna_coe_heatmap.human.all.PD.withccre.rawlinear.pdf', sep=''), width=3)
...
pca_all <- prcomp(sp_all_PD_log[,-c(1)], center = F,scale. = F)
...
eRP_mat_human[,2] = eRP_mat_human[,1]
...
plot_lim_PD = max(abs(eRP_mat_human[,1]))
pdf(paste(output_folder, '/statep_rna_coe_heatmap.human.all.PD.withccre.pdf', sep=''), width=3)
plot_lim_PD_max = max(c(abs(plot_lim_P)))
breaksList = seq(-plot_lim_PD, plot_lim_PD, by = 0.001)
my_colorbar=colorRampPalette(c('blue', 'white', 'red'))(n = length(breaksList))
col_breaks = c(seq(0, 2000,length=33))
pheatmap((eRP_mat_human[rank,1]), color=my_colorbar, breaks = breaksList, cluster_cols = FALSE,cluster_rows=F, clustering_method = 'average',annotation_names_row=TRUE,annotation_names_col=TRUE,show_rownames=TRUE,show_colnames=TRUE)
dev.off()
...
ss00 = read.table(paste('S3V2_IDEAS_hg38_ccre2.cCRE.M.withid.S0.mat.txt', sep=''), header=F)
sp_sig_pred_ct = matrix(0, nrow=dim(ss00)[1], ncol=dim(ss00)[2])
for (i in c(0:(state_n-1))){
print(i)
ss = read.table(paste('S3V2_IDEAS_hg38_ccre2.cCRE.M.withid.S',i,'.mat.txt', sep=''), header=F)
sp = (ss[,-c(1:4)])
...
ss = read.table(paste('S3V2_IDEAS_hg38_ccre2.cCRE.M.withid.S0.mat.txt', sep=''), header=F)
ss_all = (ss[,-c(1:4)])
...
sp_all_PD_corfilter_log = cbind( ((sp_corfiltered_all_D_log - mean(adj_mean_od)) / adj_sd_od * adj_sd_tar + adj_mean_tar) )


### dimension reducation rm 0 state info -c(1,25)
### PCA lm
used_idtry = dmslog_noMean>(-1000000)
sp_all_PD_corfilter_log_new = sp_all_PD_corfilter_log[,-c(1)] %*% pca0_rotation
...
colnames(eRP_mat_human) = c('P','D')
rownames(eRP_mat_human) = 0:(state_n-1)
write.table(eRP_mat_human, paste(output_folder, '/statep_rna_coe_heatmap.human.all.ccre.withcorfilter.',iter_i,'.txt', sep=''), quote=F, col.names=T, row.names=T, sep='\t')
...
pdf(paste(output_folder, '/statep_rna_coe_heatmap.human.all.PD.ccre.withcorfilter.',iter_i,'.pdf', sep=''), width=3)
...
#sp_all_PD_corfilter_log = cbind(sp_all_log, sp_corfiltered_all_D_log*global_scale)
#sp_all_PD_corfilter_log = cbind(sp_all_log + ((sp_corfiltered_all_D_log - mean(adj_mean_od)) / adj_sd_od * adj_sd_tar + adj_mean_tar) )
sp_all_PD_corfilter_log = cbind(((sp_corfiltered_all_D_log) )
...
my_colorbar=colorRampPalette(c('blue', 'white', 'red'))(n = length(breaksList))
col_breaks = c(seq(0, 2000,length=33))
pheatmap(cbind(eRP_mat_human[rank,1]), color=my_colorbar, breaks = breaksList, cluster_cols = FALSE,cluster_rows=F, clustering_method = 'average',annotation_names_row=TRUE,annotation_names_col=TRUE,show_rownames=TRUE,show_colnames=TRUE)
dev.off()
...
write.table(dmslog_noMean, paste(output_folder, '/dmslog_noMean.human.txt', sep=''), quote=F, col.names=F, row.names=F, sep='\t')
