cts = c("CFUE", "CFUMK", "CMP", "ERYfl", "GMP", "LSK", "MEP", "MONO", "NEU", "iMK", "G1E", "ER4")
### PCA lm
sig0 = read.table(paste('coe_score_no',cts[1],'/statep_rna_coe_heatmap.mouse.all.ccre.withcorfilter.txt', sep=''), header=T)

for (i in 2:length(cts)){
	sig0_i = read.table(paste('coe_score_no',cts[i],'/statep_rna_coe_heatmap.mouse.all.ccre.withcorfilter.txt', sep=''), header=T)
	sig0 = sig0+sig0_i
}
...
pdf(paste('statep_rna_coe_heatmap.mouse.all.ccre.withcorfilter.AVE.pdf', sep=''), width=3)
...
write.table(sig0, 'statep_rna_coe_heatmap.mouse.all.ccre.withcorfilter.AVE.txt', quote=F, col.names=T, row.names=T, sep='\t')
