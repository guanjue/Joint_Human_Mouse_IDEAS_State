...
write.table(dsMerge_output, '../coe_analysis_mouse/S3V2_IDEAS_mm10_ccre2.cCRE.M.notall0.withid.coe_mat.PDmerged.txt', quote=F, col.names=T, row.names=F, sep='\t')
...
png('coe_umap_PD/state_coe.mouse.umap.PD.png')
heatscatter(dss_umap$layout[,1], dss_umap$layout[,2])
dev.off()

for (i in 1:dim(dss)[2]){
	ct_i = colnames(dss)[i]
png(paste('coe_umap_PD/', 'state_coe.mouse.umap.PD.', ct_i,'.png', sep=''))
